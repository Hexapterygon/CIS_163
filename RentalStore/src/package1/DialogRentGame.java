 ...
 * allows the user to input information about the Game being rented.
 * This information includes the name of the renter, the title of 
 * the Game, the date rented, the due date, and the player type.
 * 
 * @param - none
 * @return - none
 *****************************************************************/
public class DialogRentGame extends JDialog implements ActionListener{
	...
	/*
	 * Object of the Game class used to create a Game from user entered
	 * data
	 */
	private Game unit;
	/*
	 * Drop-down menu showing the available PlayerTypes
	 */
	 ...
	 * GUIRentalStore, and accepts a blank Game to instantiate.
	 * 
	 * @param parent - JFrame used to construct the dialog window
	 * @param name - String that is set as the dialog window title
	 * @param g - Game used to store rental data 
	 * @return - none
	 *****************************************************************/
	public DialogRentGame(JFrame parent, String name, Game g){
		/*
		...
		/*
		 * Makes it so that the game for DialogRentGame and the game 
		 * passed in from GUIRentalStore point to the same address
		 */
		unit = g;
	...
	private void init(){
		...
		j.setLayout(new GridLayout(6,2,75,30));

		/*
		 * Creates a Grid layout for the label/textfield/menu panel
		 * such that there is one label and one textfield/menu in each
		 * of the five rows on the panel
		 */
		renterTxt = new JTextField();
		renterLbl = new JLabel("Your Name:");
		titleTxt = new JTextField();
		titleLbl = new JLabel("Title of Game:");
		...
		playerMenu = new JComboBox(player.values());
		playerLbl = new JLabel("Type of Player: ");
	...
	private void retrieveData(){
		 ...
		 * responsible for the invalid input.
		 */
		int textfield = 4;
		...
		if(textfield != 1 && textfield != 0){
			try{
				...
				/*
				 * Generates warning strings and passes them to the 
				 * method that creates the warning window
				 */
		...
		unit.setPlayerType((PlayerType)playerMenu.getSelectedItem());
	}
